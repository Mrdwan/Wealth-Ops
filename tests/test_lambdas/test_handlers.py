...
@pytest.fixture
def mock_boto3_dynamodb():
    """Mock boto3 dynamo client."""
    with patch("src.lambdas.data_ingestion.boto3.client") as mock:
        yield mock


def test_data_ingestion_success(mock_config, mock_boto3_dynamodb):
...
def test_data_ingestion_no_tickers(mock_config, mock_boto3_dynamodb):
...
def test_data_ingestion_partial_failure(mock_config, mock_boto3_dynamodb):
...
def test_market_pulse_success():
    """Test successful market pulse."""
    with patch("src.lambdas.market_pulse.load_config"), \
         patch("src.lambdas.market_pulse.TiingoProvider"), \
         patch("src.lambdas.market_pulse.RegimeFilter") as MockFilter, \
         patch("src.lambdas.market_pulse.TelegramNotifier") as MockNotifier:
